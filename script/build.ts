...
function copyDependencies() {
  ...
  if (isNonProductionRelease) {
    // Production releases don't ship Copilot — see the gate on our side of the
    // 3.6 merge. Upstream's helper handles the platform-specific package and
    // prebuild pruning.
...
function generateLicenseMetadata(outRoot: string) {
  ...
  const licenseWithHeader = `Madness Desktop uses licensing information provided by choosealicense.com.
